...
export default function LoginForm() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.success(`¡Bienvenido${response.user?.nombre ? `, ${response.user.nombre}` : ''}!`);
      setTimeout(() => redirectByRole(response.user?.rol), 800);
    } catch (error) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 mt-5">
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            ...
            <Link to="/forgot-password" className="text-xs text-emerald-600 hover:text-emerald-700 underline underline-offset-2">
