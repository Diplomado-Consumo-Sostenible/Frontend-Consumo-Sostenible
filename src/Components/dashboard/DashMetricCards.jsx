import { Loader2, MapPin, Sparkles, Store, TrendingUp } from 'lucide-react';
...
export default function DashMetricCards({ businesses, loadingBusinesses }) {
  ...
  const metrics = [
    {
      id: 'active', label: 'Negocios activos',
      value: loadingBusinesses ? '…' : businesses.length,
      sub: '+12% este mes', Icon: Store,
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {metrics.map((m) => (
        <div key={m.id} className="bg-card-bg rounded-2xl shadow p-4 flex items-start justify-between gap-2 overflow-hidden">
