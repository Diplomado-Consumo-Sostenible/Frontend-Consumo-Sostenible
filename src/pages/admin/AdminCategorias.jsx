...
import { Tag, Plus, Search, Edit2, Trash2, X, AlertTriangle, Loader2, LayoutDashboard, ChevronRight } from 'lucide-react';
import { getCategorias, createCategoria, updateCategoria, deleteCategoria } from '../../services/admin/categorias.service';
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar categoría..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-edge text-sm text-body placeholder-muted focus:outline-none focus:ring-2 focus:ring-primary-mid/30 focus:border-primary-mid transition-all bg-card-bg" />
      </div>
      ...
      <div className="bg-card-bg rounded-2xl border border-edge overflow-hidden shadow-warm-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-muted gap-3">
            ...
            <p className="text-sm">{search ? 'Sin resultados' : 'No hay categorías registradas'}</p>
          </div>
        ) : (
        ...
        {!loading && filtered.length > 0 && (
          <div className="px-4 py-3 border-t border-edge text-xs text-muted">
            {filtered.length} de {items.length} categorías
          </div>
        )}
      </div>
