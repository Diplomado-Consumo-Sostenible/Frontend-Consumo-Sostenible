        ...
        <div className="flex flex-col items-center text-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-stone-500 mt-1">
              ...
              <span className="font-medium text-stone-700">{user?.perfil?.nombre || user?.email}</span>?
...
function FormField({ label, error, children }) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {children}
      {error && (
        <p className="text-red-400 text-xs flex items-center gap-1"><span>⚠</span> {error}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800" style={{ fontFamily: "'Georgia', serif" }}>
            Gestión de Usuarios
          </h1>
          <p className="text-sm text-stone-400 mt-0.5">Administra los usuarios registrados en la plataforma</p>
        </div>
        <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2.5 bg-emerald-500 text-white text-sm font-medium rounded-xl hover:bg-emerald-600 transition-colors shadow-sm">
              ...
              <thead className="border-b border-stone-100 bg-stone-50/50">
                <tr>
                  ...
                  <th className={thSortClass} onClick={() => handleSort('nombre')}>
                    <span className="flex items-center gap-1">Usuario <SortIcon field="nombre" /></span>
                  </th>
                  <th className={thSortClass} onClick={() => handleSort('email')}>
                    <span className="flex items-center gap-1">Correo <SortIcon field="email" /></span>
                  </th>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition-colors disabled:opacity-50 ${user.isActive ? 'text-emerald-500 hover:bg-emerald-50' : 'text-stone-400 hover:bg-stone-100'}`}
                        >
