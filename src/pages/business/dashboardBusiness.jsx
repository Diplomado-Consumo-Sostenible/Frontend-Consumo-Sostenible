import {
    AlertCircle, AlertTriangle, Award, BarChart2, Building2,
    CalendarDays, Camera, CheckCircle2, Clock, Edit,
    Globe, ImagePlus, LayoutDashboard, Leaf, Lightbulb,
    Lock, Mail, MapPin, Phone, Tag,
} from 'lucide-react';
import { useEffect, useRef, useState } from 'react';
import { FaFacebook, FaInstagram } from 'react-icons/fa';
import { FaXTwitter } from 'react-icons/fa6';
import { Link } from 'react-router-dom';
import { getMyBusinesses } from '../../services/busienss.service';
...
function EmptyState() {
    return (
        ...
        <p className="text-sm text-muted mt-1 max-w-xs">
            Registra tu negocio para comenzar a gestionarlo desde este panel.
        </p>
        </div>
    );
}
...
function BusinessCard({ business }) {
    ...
    return (
        ...
        <div className="absolute top-4 right-4 sm:top-5 sm:right-5 flex items-center gap-1.5 flex-wrap justify-end">
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${statusCfg.bg}`}>
            {statusCfg.label}
            </span>
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${
            business.isActive
                ? 'bg-ok-bg text-ok-text border-ok-text/30'
                : 'bg-primary-softest text-muted border-edge'
            }`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-3 border-t border-edge/40">
            ...
            <div className="flex items-center gap-2 text-xs text-body min-w-0">
                ...
                className="truncate hover:text-heading transition-colors"
                >
                {business.website}
                </a>
            </div>
...
function ProfileStatus({ business, isBlocked }) {
    ...
    return (
        ...
        {isBlocked ? (
            ...
            <button
                disabled
                className="w-full text-xs font-medium py-2 px-4 bg-edge text-muted rounded-xl cursor-not-allowed opacity-60"
            >
                Completar negocio
            </button>
            </div>
...
function CertBadge() {
    ...
    return (
        ...
        <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium px-2 py-0.5 bg-ok-bg text-ok-text border border-ok-text/30 rounded-full">
            Intermedio
            </span>
        </div>
        <ul className="space-y-2">
        ...
        <Link
            to="/dashboardBusiness/certificaciones"
            className="block text-center text-xs font-medium text-primary-dark hover:text-primary-darkest transition-colors"
        >
            Ver todas →
        </Link>
        </div>
    ...
    return (
        ...
        {isBlocked ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                ...
                <div
                    key={i}
                    className="aspect-square bg-primary-softest rounded-xl flex items-center justify-center"
                >
        ...
        <div className="px-4 py-5 sm:px-6 sm:py-6 lg:pl-10 lg:pr-8 space-y-6">
            ...
            <div className="space-y-3">
                ...
                <div>
                    ...
                    Gestiona y monitorea tu negocio · Consumo Sostenible
                    </p>
                </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-card-bg rounded-2xl shadow p-5 sm:p-6 space-y-4">
                    ...
                    {QUICK_LINKS.map((item) => (
                        ...
                        key={item.to}
                        to={item.to}
                        className="flex items-center gap-3 px-4 py-3 border border-edge hover:border-primary-light hover:shadow-sm rounded-2xl transition-all group w-full sm:w-auto"
                        >
                ...
                <div className="space-y-3">
                    ...
                    businesses.map((biz) => (
                        <BusinessCard key={biz.id_business} business={biz} />
                    ))
                    )}
                </div>
